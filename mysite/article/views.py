...
from django.core.paginator import Paginator,EmptyPage,PageNotAnInteger   # 设置分页的三个方法
...
def del_article_column(request):
    ...
    column_id = request.POST['column_id']
    try:
        if ArticleColumn.objects.get(id=column_id).column == column_name:
            ArticleColumn.objects.filter(column=column_name,id=column_id).delete()
        else:
            # ArticleColumn.objects.filter(id=column_id).delete()
            pass
        return HttpResponse('1')
    except:
        return HttpResponse('0')
